...
public class C05_extends {
	...
	// # 클래스 상속
	//	- 미리 만들어 둔 클래스를 그대로 물려받아서 사용하는 것
	//	- 상속해주는 클래스를 부모 클래스 또는 super 클래스라고 한다.
	//	- 상속받는 클래스를 자식 클래스 또는 sub 클래스라고 한다.
	//	- 자식 클래스는 부모 클래스가 가진 모든 기능을 그대로 물려받는다.
	//	- 자식 클래스는 부모에게 물려받은 기능을 마음대로 수정해서 쓸 수 있다.
	//	  기능을 마음대로 수정해서 쓸 수 있다. (오버라이드, override) 		※ 오버로드와는 다른 개념이다.
	...
	public static void main(String[] args) {
		...
		// Police는 부모인 Person의 모든 정보를 가지고있기 때문에
		// 아무문제 없이 Person타입이 될 수 있다. (업캐스팅)
		// ※ 부모타입으로 올라가는 업캐스팅은 아무 문제 없다.
		
		// Person은 자식인 Police에 모르는 기능이 존재할 수 있기 때문에
		// 자식타입이 될 수 없다. (다운캐스팅)
		// ※ 다운캐스팅은 특별한 경우를 제외하고는 모두 불가능하다.
		
		Person person01 = new Police();		//	-> 업캐스팅 가능
		Person person02 = new Dentist();
		// Police police02 = new Person();	//	-> 다운캐스팅 불가능
